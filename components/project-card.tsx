...
export default function ProjectCard({ project }: { project: IProject }) {
  return (
    <div className="project-card rounded-lg shadow-md p-6 hover:shadow-lg transition-shadow duration-300 border border-white/10 hover:border-blue-500/60">
      <div className="text-accent mb-4">
        {iconMap[project.icon] || <Briefcase className="w-8 h-8" />}
      </div>
      
      <h3 className="text-xl font-bold mb-2">{project.title}</h3>
      <p className="text-sm project-card-meta mb-2">{project.companyName} • {project.year}</p>
      <p className="mb-4 project-card-description">{project.description}</p>
      
      <div className="flex flex-wrap gap-2 mb-4">
        {project.technologies.map((tech) => (
          <span key={tech} className="tech-badge text-xs px-2 py-1 rounded">
            {tech}
          </span>
        ))}
      </div>
      ...
      <div className="flex justify-between items-center">
        ...
        <a href={project.link} className="project-card-link hover:text-accent font-medium" target="_blank" rel="noopener noreferrer">
